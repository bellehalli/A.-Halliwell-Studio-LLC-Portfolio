...
export default function Home(){
 return <main id="top" className="full-picture">
  ...
  <div className="studio-world">
   <section className="world-sheet intro-sheet" id="work"><div className="content-shell"><div className="section-kicker"><span>01 / SELECTED WORK</span><span>DEPTH FIRST. RANGE NEXT. ↗</span></div><h2>Deep work in<br/><em>one industry first.</em></h2><p className="world-lede">The current flagship work explores hospitality and wedding venues in depth. It demonstrates strategy, responsive design, custom development and interactive systems, not the limit of who the studio can work with.</p></div></section>
   <div className="project-worlds">{projects.map(p=><ProjectShowcase project={p} key={p.slug}/>)}</div>

   <section className="world-sheet proof-world"><div className="content-shell"><div className="section-kicker"><span>PROOF / WITHOUT THE PRETENDING</span><span>INSPECT IT YOURSELF.</span></div><div className="proof-heading"><h2>The work<br/><em>is the proof.</em></h2><p>Explore the live builds, read the strategy behind them and use the interactive Lab yourself.</p></div><div className="proof-grid"><article><span>01 / LIVE BUILDS</span><h3>Not mockups.</h3><p>Open the flagship projects in a browser and experience the responsive builds.</p><div className="proof-links"><a href="https://willowlilyestate.com" target="_blank" rel="noreferrer">Willow Lily ↗</a><a href="https://www.maisonrivieredetroit.com" target="_blank" rel="noreferrer">Maison Rivière ↗</a></div></article><article><span>02 / CASE STUDIES</span><h3>Strategy you can read.</h3><p>See the problem, reasoning, build decisions and what each concept is designed to demonstrate.</p><a className="proof-single-link" href="/work">Read the work ↗</a></article><article><span>03 / LIVE CAPABILITIES</span><h3>The interface proves it.</h3><p>Try booking, commerce, quote, portal, events and lead-generation interfaces below.</p><a className="proof-single-link" href="#capabilities">Use the Lab ↘</a></article></div><p className="proof-disclosure">Willow Lily and Maison Rivière are original studio concept projects, not commissioned client work. I do not publish invented testimonials, logos or performance results.</p></div></section>
  ...
  <footer className="world-footer"><div className="footer-paper"><div className="footer-brand"><span className="logo-mark">A.</span><strong>A. HALLIWELL STUDIO</strong></div><p>Custom websites and digital systems built around what the business actually needs the internet to do.</p><div className="footer-links"><a href="mailto:hello@ahalliwellstudio.com">EMAIL THE STUDIO ↗</a><a href="#top">BACK TO TOP ↑</a></div></div></footer>
 </main>
}
